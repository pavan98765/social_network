...
def upload(request):
    if request.method == "POST":
        ...
        if image is not None:  # Check if a file is selected
...
def search(request):
    ...
    if request.method == "POST":
        ...
        for ids in username_profile:
            try:
                profile_lists = Profile.objects.get(id_user=ids)
                username_profile_list.append(profile_lists)
            except Profile.DoesNotExist:
                pass
...
def profile(request, pk):
    owner_object = User.objects.get(username=request.user.username)
    owner_profile = Profile.objects.get(user=owner_object)
...
def follow(request):
    if request.method == "POST":
        ...
        if FollowersCount.objects.filter(follower=follower, user=user).first():
            delete_follower = FollowersCount.objects.filter(
...
def blog(request):
    ...
    return render(request, "blog.html", {"user_profile": user_profile})
